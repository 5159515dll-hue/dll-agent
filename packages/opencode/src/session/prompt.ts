...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      const reviewerRuntimeMs = task.command === "dll-agent-supervisor"
        ? getReviewerRuntimeMs(task.agent)
        : 0
      const reviewerTimer = reviewerRuntimeMs > 0
        ? setTimeout(() => {
            error = new Error(`dll-agent reviewer runtime budget exceeded (${reviewerRuntimeMs}ms)`)
            taskAbort.abort()
          }, reviewerRuntimeMs)
        : undefined
      const result = yield* taskTool
        ...
        .pipe(
          ...
          Effect.ensuring(Effect.sync(() => reviewerTimer && clearTimeout(reviewerTimer))),
        )
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          // FIFO drain: 后续 tasks.pop() 是 LIFO，所以入栈时反转，让最早入队的 reviewer 最先 pop。
          if (supervisorPendingSubtasks.length > 0) {
            tasks.push(...supervisorPendingSubtasks.splice(0).reverse())
            if (profileEnabled()) {
              try {
          ...
          if (profileEnabled() && !session.parentID && lastUser.agent === "commander") {
            try {
              // 1. 成本追踪 — 包括子 session（reviewer subagent）的成本，避免面板只显示 commander 模型
              const childMessageGroups: MessageV2.WithParts[][] = []
              try {
                const children = yield* sessions.children(sessionID)
                for (const child of children) {
                  const childMsgs = yield* sessions.messages({ sessionID: child.id })
                  childMessageGroups.push(childMsgs)
                }
              } catch (childErr) {
                yield* slog.error("cost aggregation children fetch failed", { error: String(childErr) })
              }
              trackLastCall(sessionID, msgs, childMessageGroups)
              ...
              try {
                ...
                // 从 supervisor 真实指标导出 skill signals — 让 supervisor metrics
                // → skill 触发桥接闭环（不再仅依赖用户文本）。
                const sm = decision.metrics
                const signals: SkillSignal[] = []
                if (sm.repeated_tool_failure) signals.push("tool_failures_repeated")
                if (sm.tool_failures >= 3) signals.push("tool_failures_high")
                if (sm.permission_denied >= 1) signals.push("permission_denied")
                if (sm.reviewer_conflict_signal) signals.push("reviewer_conflict")
                if (sm.context_percent >= 40) signals.push("long_context")
                if (sm.final_claim && !sm.real_tool_evidence) signals.push("final_claim_no_evidence")
                if (sm.final_claim && !sm.verification_evidence) signals.push("verification_failed")
                const skillResult = activateSkills({
              ...
              // 注意：所有 subtask 推入持久队列 supervisorPendingSubtasks，
              // 由下一轮循环顶端 drain 到 tasks。避免一次推多个时被本轮 tasks.pop() 只消费 LIFO 末尾、
              // 其余在下一轮 tasks 重置时丢失（GLM/Kimi 同时触发只跑后者的根因）。
              if (decision.should_review) {
                const subtasks = generateSubtasks(decision, sessionID, msgs)
                for (const sub of subtasks) {
                  const reviewer = sub.agent as ReviewerRole
                  recordReviewerCall(
                    sessionID,
                    reviewer,
                    step + 1,
                ...
                try {
                  setQueuedReviewers(
                    sessionID,
                    supervisorPendingSubtasks.map((s) => s.agent),
                  )
                } catch {}
                yield* slog.info("supervisor queued reviewer subtasks", {
                  reviewers: decision.reviewers,
                  queueDepth: supervisorPendingSubtasks.length,
                })
              }

              // 3b. Auto-verifier: 如果 supervisor 自动注入了 verifier subtask，直接加入任务队列
              // 确保模型完成声明缺证据时，不会被跳过 — 代码层强制验证
              if (decision.verifierTask) {
                supervisorPendingSubtasks.push(decision.verifierTask)
                yield* slog.info("supervisor queued auto-verifier subtask", {
                  block_reason: "completion claim without real tool evidence",
                })
              }
            } catch (err) {
              // Supervisor 错误不应该中断主 loop
              yield* slog.error("supervisor error", { error: String(err) })
            }
          }
          // ─── end supervisor ───────────────────────────────────────────
            ...
            if (profileEnabled()) {
              try {
                ...
                const gateResult = checkEvidenceGate({
                  ...
                  hasVerificationEvidence: metrics.verificationEvidence,
                  risk: state.risk,
                ...
                if (!gateResult.passed && gateResult.synthetic_hint) {
                  ...
                  if (state.risk === "high" && !metrics.realToolEvidence) {
                    try {
                      const availableAgents = (yield* agents.list()).map((a) => a.name)
                      const verifier = buildVerifierSubtask(
                        sessionID,
                        gateResult.block_reason ?? "high-risk completion without real tool evidence",
                        availableAgents,
                      )
                      tasks.push(verifier)
                      yield* slog.info("supervisor injected verifier subtask", {
                        block_reason: gateResult.block_reason,
                        agent: verifier.agent,
                      })
                    } catch (vErr) {
                      yield* slog.error("verifier subtask injection failed", { error: String(vErr) })
                    }
                  }

                  supervisorUpdateState(sessionID, {
                    should_review: false,
                    reviewers: [],
                ...
                // Gate error should not prevent normal loop exit
                yield* slog.info("exiting loop (gate error, fallback)")
                break
              }
            } else {
              yield* slog.info("exiting loop")
          ...
          // ─── dll-agent: dispatch reviewer subtasks ────────────────────────
          ...
          if (profileEnabled() && tasks.length >= 2) {
            ...
            if (batch.length >= 2) {
              ...
              const runSingleSupervisorTask = Effect.fn("SessionPrompt.runSingleSupervisorTask")(function* (
                ...
                yield* slog.info("supervisor single dispatch", {
                  reviewer: task.agent,
                  mode,
              ...
              const runReadOnlyGroup = Effect.fn("SessionPrompt.runReadOnlySupervisorGroup")(function* (
                group: MessageV2.SubtaskPart[],
              ) {
                try {
                  setRunningReviewers(
                    sessionID,
                ...
                yield* slog.info("supervisor parallel read dispatch", {
                  count: group.length,
            ...
            // 只有 1 个 reviewer，回落到下面的单 pop 路径
            if (batch.length === 1) tasks.push(batch[0])
          }
          // ─── end dll-agent parallel dispatch ──────────────────────────────
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            // Phase 4: append a one-line dll-agent active-skills summary so commander
            // sees which skills are currently active. Persisted in active-skills.json by supervisor.
            if (profileEnabled() && !session.parentID) {
              try {
                const activeDefs = loadActiveSkills(sessionID)
                if (activeDefs.length > 0) {
                  const names = activeDefs.map((s) => `${s.name}(${s.riskLevel})`).join(", ")
                  let line = `[dll-agent active skills: ${names}]`
