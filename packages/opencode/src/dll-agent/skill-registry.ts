...
export const SKILL_REGISTRY: SkillDefinition[] = [
  ...
  // ─── Phase 6: Model Routing & Token Awareness ────────────────────────────
  {
    id: "model-routing-policy",
    name: "model-routing-policy",
    description: "Token-aware model routing: trigger Kimi/GLM on correct conditions, avoid DeepSeek overload. Evidence-gated.",
    version: "1.0.0",
    riskLevel: "medium",
    triggers: {
      keywords: [/多模型|model.*routing|kimi.*trigger|glm.*trigger|token.*budget|context.*compress|continuation.*packet/i],
      signals: ["long_context"],
    },
    activationPolicy: { maxActivationsPerSession: 5, minStepInterval: 2 },
    deactivationPolicy: { onPhaseChange: true },
    ...
    allowedCommands: [],
    forbiddenCommands: [],
    requiredEvidence: ["routing_decision", "why_kimi_triggered", "why_glm_triggered", "why_cross_review_triggered"],
    verificationCommands: [],
    cooldown: { minIntervalSec: 30, fingerprintIntervalSec: 120 },
    costPolicy: { singleCallCapMultiplier: 0.5, allowOpenAI: false, allowExpensiveReviewer: false },
    securityPolicy: { requireRedaction: true, allowExternalWrite: false, allowNetworkFetch: false },
    rollbackPolicy: { autoRollback: false, preRollbackChecks: [], rollbackCommands: [] },
  },
  {
    id: "kimi-continuation-trigger",
    name: "kimi-continuation-trigger",
    description: "Kimi task-completion-archivist trigger: pre-final-report check, continuation packet generation, handoff packet. Blocks final PASS when unfinished work remains.",
    version: "1.0.0",
    riskLevel: "high",
    triggers: {
      keywords: [/final report|完成报告|finalization|continuation|handoff|unfinished|未完成|blocking.*unfinished/i],
      signals: ["final_claim_no_evidence"],
    },
    activationPolicy: { maxActivationsPerSession: 3, minStepInterval: 2 },
    deactivationPolicy: { onVerificationDone: true },
    requiredTools: ["task"],
    allowedCommands: [],
    forbiddenCommands: [],
    requiredEvidence: ["continuation_packet", "blocking_unfinished_items", "next_execution_plan"],
    verificationCommands: [],
    cooldown: { minIntervalSec: 30, fingerprintIntervalSec: 120 },
    costPolicy: { singleCallCapMultiplier: 0.3, allowOpenAI: false, allowExpensiveReviewer: false },
    securityPolicy: { requireRedaction: true, allowExternalWrite: false, allowNetworkFetch: false },
    rollbackPolicy: { autoRollback: false, preRollbackChecks: [], rollbackCommands: [] },
  },
  {
    id: "glm-requirements-trigger",
    name: "glm-requirements-trigger",
    description: "GLM requirements-inspector trigger: completion-claim-check, scope-expansion, partial-as-complete detection, Chinese requirement alignment.",
    version: "1.0.0",
    riskLevel: "medium",
    triggers: {
      keywords: [/complete|done|finish|完成|完工|scope.*creep|需求.*漂移|纠偏|correction/i],
      signals: ["final_claim_no_evidence", "verification_failed"],
    },
    activationPolicy: { maxActivationsPerSession: 3, minStepInterval: 2 },
    deactivationPolicy: { onPhaseChange: true },
    ...
    allowedCommands: [],
    forbiddenCommands: [],
    requiredEvidence: ["completion_claim_check", "scope_expansion_check", "requirement_alignment"],
    verificationCommands: [],
    cooldown: { minIntervalSec: 30, fingerprintIntervalSec: 120 },
    costPolicy: { singleCallCapMultiplier: 0.3, allowOpenAI: false, allowExpensiveReviewer: false },
    securityPolicy: { requireRedaction: true, allowExternalWrite: false, allowNetworkFetch: false },
  ...
  {
    id: "cross-review-escalation-policy",
    name: "cross-review-escalation-policy",
    description: "Cross-review escalation: trigger when same failure fingerprint repeats >=2, reviewer conflict, high-risk completion without evidence, or scope expansion.",
    version: "1.0.0",
    riskLevel: "high",
    triggers: {
      keywords: [/cross[- ]?review|escalat|升级.*审查|重复.*失败|same.*failure.*pattern/i],
      signals: ["reviewer_conflict", "tool_failures_repeated"],
    },
    activationPolicy: { maxActivationsPerSession: 2, minStepInterval: 5 },
    deactivationPolicy: { onPhaseChange: true, onVerificationDone: true },
    requiredTools: ["task"],
    allowedCommands: [],
    forbiddenCommands: [],
    requiredEvidence: ["escalation_reason", "cross_review_council_composed", "conflict_resolution"],
    verificationCommands: [],
    cooldown: { minIntervalSec: 120, fingerprintIntervalSec: 300 },
    costPolicy: { singleCallCapMultiplier: 1.0, allowOpenAI: true, allowExpensiveReviewer: true },
    securityPolicy: { requireRedaction: true, allowExternalWrite: false, allowNetworkFetch: false },
    rollbackPolicy: { autoRollback: false, preRollbackChecks: [], rollbackCommands: [] },
  },
