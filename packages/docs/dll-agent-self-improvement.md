...
- `full-access`: 当前兼容默认值，commander/executor 工具请求经 role policy 检查后自动允许；read-only reviewer 的写入/命令权限仍被拒绝。
...
| Permission modes | `permission-mode.ts` + `/permissions` + TUI command palette | 三档模式：Default 使用 OpenCode 默认权限流；Auto-review 自动放行低风险/commander 项目内普通写入并询问高风险；Full Access 为当前兼容默认，动态放行 commander/executor 工具请求且仍保留 read-only reviewer deny |
| Risk-based permission precheck | `permission-bridge.ts` + `permission-classifier.ts` + `role-tool-policy.ts` | Auto-review 模式下高风险 push/sudo/secrets/destructive 仍 ask；Full Access 模式下经 role policy 后放行；read-only reviewer 写入 deny |
...
| high-risk confirmation | implemented_runtime_verified | Auto-review 模式下 `git push`、`sudo`、`rm -rf`、secret file access 仍 ask；Full Access 按用户显式选择放行 commander/executor，但 reviewer read-only deny 不变 |
