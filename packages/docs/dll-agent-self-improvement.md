...
| Live scenario execution | partial_runtime | Phase 10 当前是 deterministic/local regression dashboard，不自动跑 20 个真实 live 任务；MiMo 多模态 live 和需要用户凭据/授权的场景标记为 live_not_run/manual_not_run，后续可单独做 Phase 10.1 harness |
...
| MiMo Provider Status | implemented_runtime_verified | status/quota unavailable/no endpoint 可见；live multimodal 仍未跑 |
...
| multimodal-context-interpreter | implemented_deterministic_verified | detection、packet、ResultPacket、redaction 已测；live MiMo screenshot 未跑 |
...
| Regression scenarios | implemented_deterministic_verified | 20/20 deterministic/local pass；manual/live 仍 not_run |
...
| 场景 | 状态 | 说明 |
|---|---|---|
| Manual secrets/permission scenario | manual_not_run | 需要用户授权和手工确认，不在 RC 自动执行 |
| Live multimodal screenshot scenario | live_not_run | 需要 live MiMo/多模态输入，不在 RC 自动执行 |
