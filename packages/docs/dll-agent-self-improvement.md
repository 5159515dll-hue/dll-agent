...
| Capability → Skill 动态解析 | `capability-skill-resolver.ts` + `tool/skill.ts` | 不维护源码 alias 表；`ppt-pptx:tool` 等请求通过 tool catalog / capability metadata / 已安装 skill 名称与描述动态解析，新增工具只需 manifest/catalog 声明 |
| Multi-capability workflow planning | `capability-planner.ts` | 任务可同时选中 primary/support/validation/fallback 能力；组合依据 registry metadata、输入输出关系、验证声明和缺口，不把单个 skill/MCP/software 当成唯一答案 |
...
| 工具系统测试 | `test/dll-agent/tools.test.ts` | 覆盖 catalog、overlay/merge、prompt injection、MCP mutex、doctor、evidence，以及 metadata-driven skill resolution |
...
| Answer Delivery lifecycle | ✅ 本轮实现 | `answer-delivery.ts` 为 L0/L1/L2 answer-only 任务建立公开输出锁：第一个被接受的 commander 答案写入 `answer_delivery.status=accepted`，后续 recovery/reviewer/supervisor 文本不能再次对同一用户消息输出公开总结；L3/L4、纠偏、重复失败、reviewer conflict、权限和高风险任务仍走原有多模型会审与 gate |
...
Answer Delivery is the boundary between internal governance and user-visible
responses. It fixes the repeated-summary failure mode where a read-only request
could receive a commander answer, then a recovery answer, then reviewer/final
answers for the same user message.

Runtime behavior:

- L0/L1/L2 answer-only tasks can be accepted after the commander response when
  the tool summary contains only read-only tools and no blocking failure.
- Accepted state records `public_answer_emitted=true` and
  `public_followup_allowed=false`.
- Queued supervisor subtasks, task-completion-archivist, final-auditor, and
  executor auto-verifier are suppressed for that same user message.
- Read-only tools such as Read/Glob/List are allowed evidence gathering for L2;
  shell commands, write tools, MCP use, or non-read-only failures prevent
  answer-only acceptance.
- The mechanism does not weaken engineering correctness: code changes,
  debugging, explicit verification, user correction, repeated failure, reviewer
  conflict, final evidence gaps, permission/secrets/destructive operations,
  doctor failed, and L4 high-risk work still use the existing role model,
  recovery, cross-review, Result Ledger, and final gate paths.

The intended user-facing invariant is one accepted answer for one ordinary
answer-only user request. Internal model comparison and council mechanisms may
still run for tasks that actually need them, but they should converge into one
accepted public result rather than produce multiple competing public summaries.
...
| acquisition instead of skip | implemented_runtime_verified | Missing tokens now produce `ask_permission`; missing no-install strategy capabilities request acquisition/configuration; unresolved gaps become explicit `capability-gap:*` actions instead of silent skip/degrade |
| real download/install/start | partial_runtime | Static low-risk download path exists and low-risk registered project-local installs can run through guarded `auto_install`; arbitrary external execution, global install, MCP start, Playwright start, and unverified activation remain blocked |
...
| 特性 | 说明 |
|---|---|
| 无边界自动安装外部服务 | 不在范围内；低风险注册能力可自动安装，高风险/未知来源必须走 acquisition、审计或授权 |
